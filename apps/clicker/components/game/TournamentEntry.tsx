...
export function TournamentEntry({
  ...
  if (status === "entered") {
    return (
      <div className="rounded-xl border-2 border-accent/60 bg-accent/5 p-4">
        <p className="display text-h3 text-accent-deep">
          🌳 Entered — Tournament #{tournamentId.toString()}
        </p>
        <p className="mt-1 text-sm text-muted">You are in. Grow on.</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col gap-3 rounded-xl border-2 border-border bg-surface p-4 shadow-[0_6px_16px_rgba(62,44,34,0.08)]">
      <div>
        <h3 className="display text-h3 text-accent-deep">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <button
          ...
          className="min-h-[40px] flex-1 rounded-lg border-2 border-border bg-surface-2 px-3 py-2 text-sm font-bold text-fg hover:border-accent disabled:opacity-40"
        >
        ...
        <button
          ...
          className="min-h-[40px] flex-1 rounded-lg bg-accent px-3 py-2 text-sm font-bold text-white shadow-[0_6px_16px_rgba(90,132,92,0.35)] hover:bg-accent-deep disabled:opacity-40 disabled:shadow-none"
        >
...
function StatusRow({
  ...
  return (
    <div className="grid grid-cols-2 gap-x-2 gap-y-1 rounded-lg border border-border bg-surface-2 p-2 text-xs">
      <span className="text-muted">Balance</span>
      <span className="text-right text-fg tabular">{fmt(balance)}</span>
      <span className="text-muted">Allowance</span>
      <span className="text-right text-fg tabular">{fmt(allowance)}</span>

      {status === "no_balance" && (
        <p className="col-span-2 mt-1 rounded bg-sunshine/20 p-2 text-fg">
