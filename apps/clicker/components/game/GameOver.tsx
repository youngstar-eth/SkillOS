...
export function GameOver({
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Season finished"
      className="fixed inset-0 z-50 flex items-center justify-center bg-fg/30 p-4 backdrop-blur-sm"
    >
      <div className="w-full max-w-sm rounded-xl border-2 border-bark/50 bg-surface p-5 shadow-[0_16px_48px_rgba(62,44,34,0.25)]">
        <h2 className="display text-h2 text-accent-deep">The season ends 🍂</h2>
        <p className="mt-1 text-sm text-muted">
          Final score:{" "}
          <span className="display text-h2 text-leaf tabular">
            {finalScore.toLocaleString()}
          </span>
        </p>

        <div className="mt-4 grid grid-cols-3 gap-3 rounded-lg border border-border bg-surface-2 p-3 text-xs">
          <Stat label="Leaves" value={formatNumber(totalLeavesEarned)} />
          <Stat label="Clicks" value={totalClicks.toLocaleString()} />
          <Stat label="Upgrades" value={upgradesOwned} />
        </div>
        ...
        <div className="mt-5 flex flex-col gap-2">
          <button
            type="button"
            onClick={onRestart}
            className="min-h-[44px] w-full rounded-lg border-2 border-border bg-surface text-sm font-bold text-fg hover:border-accent"
          >
            Plant again
          </button>
          <button
            ...
            className="min-h-[44px] w-full rounded-lg bg-accent text-sm font-bold text-white shadow-[0_6px_16px_rgba(90,132,92,0.4)] hover:bg-accent-deep disabled:opacity-40 disabled:shadow-none"
          >
            {buttonLabel}
          </button>
        </div>
        ...
        {submit.status === "done" && (
          <div className="mt-4 rounded-lg border border-success/50 bg-success/10 p-3 text-xs">
            <p className="font-bold text-accent-deep">✓ Score submitted on-chain</p>
...
function Stat({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="font-bold text-fg tabular">{value}</span>
    </div>
  );
}
