...
import { MAX_SUBMITTABLE_SCORE } from "@/lib/clicker/engine";
...
export default function DuelPage({ params }: PageProps) {
  ...
  const submit = useCallback(
    async (score: number) => {
      ...
      setSubmitting(true);
      setFrozen(true);
      // Clicker-specific bracket: the shared backend accepts scores in
      // (0, 50000). Floor to 1 so a "never tapped" timer expiry still
      // submits; ceiling to MAX_SUBMITTABLE_SCORE so an autoclicker
      // spike above 49_999 records the max accepted value instead of
      // triggering a 400 implausible_score. Anti-cheat proper is V2.
