...
export default function DuelPage({ params }: PageProps) {
  ...
  const submit = useCallback(
    async (score: number) => {
      ...
      setSubmitting(true);
      setFrozen(true);
      // Minesweeper-specific floor: liveScore is 0 before the first reveal.
      // If the timer expires before any click, submit(0) would hit the
      // backend's score > 0 validation. Floor to 1 so a "stared at the
      // board but didn't move" submission is still recorded.
