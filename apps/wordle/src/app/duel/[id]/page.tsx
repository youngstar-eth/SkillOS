...
export default function DuelPage({ params }: PageProps) {
  ...
  const submit = useCallback(
    async (score: number) => {
      ...
      setSubmitting(true);
      setFrozen(true);
      // Wordle-specific floor: the shared backend rejects score <= 0. Timer
      // expire fires submit(liveScore) and liveScore stays at 0 for Wordle
      // (the Board itself is the progress signal, not the ScoreCard). Floor
      // to 1 so the backend accepts a "played but didn't finish" submission.
