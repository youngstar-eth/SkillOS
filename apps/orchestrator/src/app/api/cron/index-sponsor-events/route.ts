...
// Schedule defined in apps/orchestrator/vercel.json. Daily (00:15 UTC, offset
// from the tournament + anchor crons so they don't pile up). Vercel Hobby tier
// rejects sub-daily expressions; sub-daily cadence requires Pro upgrade or an
// external scheduler hitting this endpoint with the bearer token.
//
// Until upgraded: sponsor events accumulate up to 24h between indexer runs.
// Acceptable for sweepstakes posture (no funds at risk during the gap), worse
// UX (sponsor's dashboard reflects with up to 24h lag).
//
