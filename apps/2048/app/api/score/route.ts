...
import { verifyBearer } from "@/lib/auth/quick-auth";
import { createAdminSupabase } from "@/lib/supabase/server";
import { signScore } from "@/lib/signing/score-signer";
import { ARCADE_POOL_ADDRESS } from "@/lib/contracts/arcade-pool";
...
export async function POST(req: NextRequest) {
  ...
  // tournamentId is a bigint on-chain but arrives as number|string|bigint-string
  ...
  // Look up the user by fid — this is the trusted source of walletAddress.
  ...
  try {
    ...
    return NextResponse.json({ error: "signer_failed", detail: message }, {
      status: 500,
    });
  }
}
