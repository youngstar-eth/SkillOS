...
OYUN: Jetpack Joyride
TOURNAMENT ID: 15
PORT: 3015
WORKTREE: /Users/inancayvaz/MAS-jetpack
BRANCH: game/jetpack
DESIGN: Cybercore
...
export function createInitialState(seed: number): JetpackState {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (isHazard) {
      ...
      const type: HazardType = r < 0.5 ? 'laser-h' : r < 0.85 ? 'laser-v' : 'missile'
      rng = Math.imul(rng, 2654435761) >>> 0
      const y = CEILING_Y + (rng / 0x100000000) * (FLOOR_Y - CEILING_Y - 100)
      hazards.push({
        x, y,
      ...
      rng = Math.imul(rng, 2654435761) >>> 0
      const y = CEILING_Y + (rng / 0x100000000) * (FLOOR_Y - CEILING_Y - 40)
      // 3 coin row
      for (let k = 0; k < 3; k++) coins.push({ x: x + k * 30, y, collected: false })
    }
    rng = Math.imul(rng, 2654435761) >>> 0
    x += 160 + (rng / 0x100000000) * 150
  }
  return {
    playerY: BOARD_HEIGHT / 2, playerVy: 0,
    thrusting: false, hazards, coins,
...
export function tick(state: JetpackState, dt: number): JetpackState {
  ...
  if (y < CEILING_Y + PLAYER_RADIUS) { y = CEILING_Y + PLAYER_RADIUS; vy = 0 }
  if (y > FLOOR_Y - PLAYER_RADIUS) { y = FLOOR_Y - PLAYER_RADIUS; vy = 0 }

  const hazards = state.hazards.map(h => ({ ...h, x: h.x - state.speed * frame }))
    .filter(h => h.x > -200)
  const coins = state.coins.map(c => ({ ...c, x: c.x - state.speed * frame }))
    .filter(c => c.x > -50)

  // Collision: hazards
  for (const h of hazards) {
    if (PLAYER_X + PLAYER_RADIUS > h.x && PLAYER_X - PLAYER_RADIUS < h.x + h.width) {
  ...
  return {
    ...state, playerY: y, playerVy: vy,
    hazards, coins, coinsCollected,
...
createInitialState: hazards + coins array
setThrust on/off
tick: gravity without thrust
tick: thrust reduces vy
tick: ceiling clamp
tick: floor clamp
tick: hazard collision → gameOver
tick: coin collect
tick: offscreen objects filtered
tick: speed increases
calculateScore: distance + coins
max vy clamp

═══ ADIM 3: UI ═══

Canvas 800×500
Cyber-grid bg (scrolling)
Player: humanoid with jetpack flame (thrusting = bigger flame)
Hazards:
  laser-h = horizontal neon line, pulsing
  laser-v = vertical line
  missile = triangular missile shape
Coins: gold circle with $ icon

Controls: Tap/hold-Space = thrust

Tournament ID: 15n

═══ ADIM 4: DESIGN — Cybercore ═══

:root {
  --color-bg: 8 10 25;
  --color-fg: 220 240 255;
  --color-accent: 0 240 255;
  --color-accent-2: 255 0 220;
  --color-gold: 255 210 0;
  --color-danger: 255 40 80;
...
body {
  background:
    linear-gradient(180deg, rgb(var(--color-bg)), rgb(20 5 40)),
    radial-gradient(ellipse at 50% 0%, rgba(0,240,255,0.15), transparent 60%);
